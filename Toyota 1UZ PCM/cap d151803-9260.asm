...
count_A7:	.block 1		; DATA XREF: __RESET+2C1r __RESET+567w
					; saturates rapidly when IDL1 is not set, incremented by F47D
...
byte_199:	.block 1		; DATA XREF: __RESET:main_35w
					; sub_F1A9+6Aw	...
					; output from 3d table c54c, less with RPM, more with load
...
byte_C2DB:	.db 018			; data used by F070 (18)
		...
		.db  2Ch ; ,
		.db 0FFh
unk_C2EA:	.db  86h ; Ü		; C5F4,	lookup for rawTHW
		.db  14h		; 5120
		.db 0D2h ; “
		.db  0Ah		; 2560
		.db 0FFh
...
main_10:				; CODE XREF: __RESET+237j
		...
		bgt	main_11
		ld	d, #0A000h	; saturate RPM at 8000
...
main_13:				; CODE XREF: __RESET+246j
		...
		ei
		cmpb	b, #40h		; check	the previous value of flags_77 for bit6
		beq	main_14
...
main_17:				; CODE XREF: __RESET+2AEj
		...
		ld	y, #7800h	; assert minimum value for closed throttle or bad tps measurement(?)
...
main_34:				; CODE XREF: __RESET+3D9j
		...
		ld	d, RPM		; MSB is RPM/50, LSB is	fraction of 50
		jsr	ThreeDtable	; less with RPM, more with load
		tbbc	bit3, flags_45,	main_35	; store	table output
		add	a, #043

main_35:				; CODE XREF: __RESET+3F3j
		st	a, byte_199	; store	table output
...
main_65:				; CODE XREF: __RESET+54Fj __RESET+554j ...
		...
		clr	count_A7	; saturates rapidly when IDL1 is not set, incremented by F47D
		cmp	a, #7Ah
		bcs	main_66
		setb	bit7, flags_44
...
fuel_120:				; CODE XREF: __RESET+846j __RESET+849j ...
		...
		jmp	fuel_144	; assign pulsewidth
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ
...
fuel_122:				; CODE XREF: __RESET+863j
		...
		ld	y, #0C2A8h
		ld	a, unk_DB	; engine temp related, as well as air temp and baro
		jsr	TwoDTable	; X value in AccA, integer return in Acca, fraction return in AccB
		push	d
		ld	d, RPM		; MSB is RPM/50, LSB is	fraction of 50
		shr	d
		...
		mov	d, x
		pull	d
		cmp	x, #00256	; 12.5 RPM, x is now 0.0488288 rpm/bit
		bcs	fuel_120	; bounce if barely running
		jsr	divDbyX		; enrichment factor which decreases with RPM?
		shl	d
		bcc	fuel_123	; end alpha-N routine
		ld	d, #0FFFFh

fuel_123:				; CODE XREF: __RESET+894j
		mov	d, y		; end alpha-N routine
...
fuel_139:				; CODE XREF: __RESET+957j
		...
		nop
		nop
		ld	d, word_12E	; closed loop fuel adjustments?
		tbbc	bit0, flags_40,	fuel_140 ; B3 :	forced timing to 5/10 deg BTDC
		ld	d, word_14A	; closed loop fuel adjustments (other bank)?
...
fuel_140:				; CODE XREF: __RESET+96Bj
		...
		pull	y		; pushed from dd62, this is compLOAD based pulsewidth !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
		bcs	fuel_141
		jsr	MulDbyY		; returns upper	16 bits	in Y, lower 16 bits in D
...
fuel_141:				; CODE XREF: __RESET+97Cj
		...
		bcs	fuel_142	; engine running too slow, limit need not be applied
		cmp	d, temp_51
		bcc	fuel_142	; bounce if not	"limp in" injection mode - normal execution path
		st	d, temp_51	; limit	injector pw to 2 crank revolutions

fuel_142:				; CODE XREF: __RESET+987j __RESET+98Aj ...
		tbbc	bit1, flags_46,	fuel_145 ; bounce if not "limp in" injection mode - normal execution path
		di
...
fuel_143:				; CODE XREF: __RESET+9A0j
		...
		jsr	MulAbyY		; returns upper	16b in AccD
		cmp	d, temp_51
		bcc	fuel_144	; bounce if 90%	of InjectPW1 is	bigger than unk_51
		clrb	bit1, flags_46
		ld	d, temp_51	; select unk_51	because	it was bigger

fuel_144:				; CODE XREF: __RESET+856j __RESET+9AFj
...
main_152:				; CODE XREF: __RESET+9F4j
		...
		cmp	a, b
		beq	main_153
		cmp	a, unk_DB	; engine temp related, as well as air temp and baro
		beq	main_153
		mov	b, a
		cmp	a, unk_DB	; engine temp related, as well as air temp and baro
		beq	main_153
...
main_153:				; CODE XREF: __RESET+A04j __RESET+A08j ...
		...
		mov	a, b
		add	b, #032
		st	b, unk_DC	; engine temp related, as well as air temp and baro
		setb	bit3, flags_48
		setb	bit4, flags_48
...
main_157:				; CODE XREF: __RESET+A2Fj
		...
		clrb	bit1, flags_48
		clrb	bit2, flags_46
		ld	#0ACh, unk_DC	; engine temp related, as well as air temp and baro
		bra	main_161
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ

main_158:				; CODE XREF: __RESET+A49j
		cmp	#9Dh, unk_DC	; engine temp related, as well as air temp and baro
		bgt	main_161
		cmp	#7Dh, unk_DB	; engine temp related, as well as air temp and baro
		bne	main_159
...
main_159:				; CODE XREF: __RESET+A5Fj
		inc	unk_DC		; engine temp related, as well as air temp and baro
		bra	main_161
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ
...
main_164:				; CODE XREF: __RESET:main_163j
		...
		mov	a, b
		add	b, #032
		st	d, unk_DB	; engine temp related, as well as air temp and baro
		mov	a, b
		xor	b, #0FFh
		st	d, word_80
...
main_169:				; CODE XREF: __RESET:main_166j
		...
		ld	d, unk_DB	; engine temp related, as well as air temp and baro
		add	a, #20h
		cmp	a, b
		bne	main_172
		setb	bit1, flags_48
...
main_206:				; CODE XREF: __RESET:main_205j
		dec	unk_DC		; engine temp related, as well as air temp and baro
		bra	main_207
...
main_208:				; CODE XREF: __RESET+C36j __RESET+C69j
		...
		add	a, #20h
		cmp	a, unk_DC	; engine temp related, as well as air temp and baro
		beq	main_210
		bcs	main_209
		inc	unk_DC		; engine temp related, as well as air temp and baro
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ
...
main_226:				; CODE XREF: __RESET+D38j
		neg	b
		cmp	b, unk_DC	; engine temp related, as well as air temp and baro
		bcc	main_229
		dec	unk_DC		; engine temp related, as well as air temp and baro
		cmp	a, #16h
		ble	main_227
...
main_228:				; CODE XREF: __RESET+D20j __RESET+D30j
		...
		ld	a, temp_51
		sub	a, #20h
		sub	a, unk_DB	; engine temp related, as well as air temp and baro
		beq	main_233
		bcs	main_230
...
main_236:				; CODE XREF: __RESET+CA3j
		...
		add	a, #20h
		mov	a, b
		sub	a, unk_DC	; engine temp related, as well as air temp and baro
		bcc	main_237
		neg	a
...
IV6_6:					; CODE XREF: IV6+1Fj IV6+2Aj ...
		clr	a
		div	d, #03h
		st	d, temp_58	; temp for IV6
		ld	a, temp_58	; temp for IV6
		shr	a
		bcc	IV6_14
...
IV6_36:					; CODE XREF: IV6+18Aj
		...
		and	b, #3Fh
		bne	IV6_37
		ld	a, flags_77	; clear	bit 4
		and	a, #0EFh
		st	a, flags_77	; clear	bit 4
...
loc_F200:				; CODE XREF: sub_F1A9+47j sub_F1A9+4Cj ...
		...
		cmpb	a, #80h
		bne	loc_F21E
		st	b, byte_199	; output from 3d table c54c, less with RPM, more with load
...
loc_F2C1:				; CODE XREF: sub_F1A9+115j
		...
		ld	a, unk_198
		add	a, b
		ld	b, byte_199	; output from 3d table c54c, less with RPM, more with load
		sub	b, unk_19A
		bcc	loc_F2D2
		clr	b
...
loc_F2DB:				; CODE XREF: sub_F1A9+105j
		...
		add	a, #02h
		bcs	loc_F2F3
		cmp	a, byte_199	; output from 3d table c54c, less with RPM, more with load
		ble	loc_F2F6
...
loc_F504:				; CODE XREF: IV6+599j
		...
		shr	a
		shr	a
		bcc	loc_F559	; every	131ms
		shr	a
		bcc	loc_F522	; every	262ms
		shr	a
		bcc	loc_F516	; every	524ms
		shr	a
		bcc	loc_F519	; every	1.048s
...
loc_F519:				; CODE XREF: IV6+5B0j
		ld	a, flags_77	; clear	bits 1,2
		and	a, #0F9h
		st	a, flags_77	; clear	bits 1,2
		jmp	loc_F666
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ

loc_F522:				; CODE XREF: IV6+5AAj
		ld	a, flags_77	; clear	bit 3
		and	a, #0F7h
		st	a, flags_77	; clear	bit 3
		bra	loc_F52E
...
loc_F5AE:				; CODE XREF: IV6+642j
		ld	d, unk_DC	; engine temp related, as well as air temp and baro
		cmpz	b
		beq	loc_F5D9
		bpz	loc_F5C4
		cmp	b, #0D2h
...
loc_F5E2:				; CODE XREF: IV6:loc_F5D9j IV6+67Aj
		...
		bcs	loc_F5FF
		ld	x, #0C527h
		cmp	#78h, unk_DB	; engine temp related, as well as air temp and baro
		bcs	loc_F5FF
		ld	x, #0C51Eh
		bra	loc_F5FF
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ
...
; ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ
		.db 0C1h ; ¡		; data for F3DE, d2d3 start
		.db 0C3h ; √
